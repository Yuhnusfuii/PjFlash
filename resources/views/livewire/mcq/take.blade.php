{{-- resources/views/livewire/mcq/take.blade.php --}}
  ...
  @elseif ($finished)
    <div class="y-card y-card-pad text-center space-y-4">
      ...
      <div class="flex items-center justify-center gap-3">
        <button type="button" wire:click="retry" class="y-btn y-btn--brand">Làm lại</button>
  ...
  @else
    <div class="y-card y-card-pad space-y-5">
      <div class="flex items-center gap-2 text-sm text-slate-500">
        <span>Câu {{ $loopIndex = (int)strtok($progress, '/') }} / {{ $total }}</span>
      </div>
      ...
      <div class="grid md:grid-cols-2 gap-3">
        @foreach (($q['options'] ?? []) as $idx => $opt)
          @php
            ...
            $classes = 'y-input text-left h-12 flex items-center rounded-2xl border transition hover:bg-slate-50 dark:hover:bg-slate-800/60';
            if (isset($picked)) {
                $classes .= $isCorrect
          ...
          <button type="button" class="{{ $classes }}" wire:click="choose({{ $idx }})">
      ...
      <div class="flex items-center justify-between pt-2 text-sm">
        <button type="button" class="text-slate-500 hover:underline" wire:click="retry">Retry</button>
        <button type="button" class="y-btn y-btn--brand" wire:click="next" @disabled(!isset($picked))>
          {{ ($loopIndex ?? 1) < $total ? 'Next' : 'Finish' }}
        </button>
      </div>
    </div>
  @endif
